...
class Hook{
	...
	Default:
		simpex
			callname':'descripter
		example
			ModelApi::standardHandling:post
		descripter standards
			before
				before any code in function is run
			after
				after all code in function is run
			post
				after some expected behavior within the function.  Can have additional descrptors
					example
						ModelApi::doCreate:pre
						ModelApi::doCreate:pre:create


	++ }
	#++ Callback standards {
	returning false will remove the hook

	++ }
	@param	spotName	see spot name standards ^
	@param	callback	what to run on hook.  See callback standards ^
	@param	deleteAfter : <int><<deletes hook after # of uses>>
	*/
	...
	static function run($spotName,&$a1=null,&$a2=null,&$a3=null,&$a4=null,&$a5=null,&$a6=null,&$a7=null,&$a8=null,&$a9=null,&$a10=null,&$a11=null,&$a12=null,&$a13=null){
		...
		return $returns;	}
	static function has($spotName){
		return (bool)self::$hooks[$spotName];	}
