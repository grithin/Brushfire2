...
class AutoloadPublic{
	...
	*/
	...
	function cache($className){
		if($path = Cache::get('_autoload.'.$className)){
			require_once($path);
			return;
		}
		
		$path = $this->req($className);
		if($path){
			Cache::set('_autoload.'.$className,$path);
			//not too concerned about locking here
			$cached = Cache::get('_autoloadCached');
			$cached[] = $className;
			Cache::set('_autoloadCached',$cached);
		}
	}
	...
	@return	path on first load,, otherwise found=>null.  If not found, found=>false
	*/
	function load($className){
		$excludePaths = array();
		//go throught all possible paths until either finding the class or failing
		while(!class_exists($className,false) && !trait_exists($className,false)){
	...
	function loaded($className){
		$result = $this->load($className);
		return $result['found'] === false ? false : true;
	}
